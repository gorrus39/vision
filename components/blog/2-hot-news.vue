...
const hot_items_desktop_limited = computed(() => hot_items_by_lang.value.slice(0, 3));
const hot_items_mobile_limited = computed(() => hot_items_by_lang.value.slice(0, 4));
</script>
...
<template>
  <div class="mb-M-80 me-D-155 ms-D-156 md:mb-D-95">
    ...
    >
      <HomeBlogItemCopy v-for="item in hot_items_desktop_limited" :blogItem="item" hotItem />
    </div>

    <div :class="[`border-M-s border-M-t border-M-e flex flex-col border-white md:hidden`]">
      <HomeBlogItemCopy v-for="item in hot_items_mobile_limited" :blogItem="item" mobile hotItem />
    </div>
  </div>
</template>
