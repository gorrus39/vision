...
<template>
  <div class="space-y-4" v-if="item">
    ...
    <div class="mt-4" v-if="item?.img_path?.length && item?.img_path?.length > 0">
      <img
        class="w-D-400 rounded-lg border object-cover shadow-md"
        :src="getCatalogItemImageUrl(item.img_path)"
        alt="Photo"
      />
      <UButton
