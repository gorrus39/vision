...
import { catalogAdminSchema, type CatalogAdmin, type Lang } from "~/types/catalog"
import { provide } from "vue"
import type { Image } from "~/types/common"
import { UButton } from "#components"

const store = useCurrentStore("catalogAdminsStore")
...
provide<FormEntries<CatalogAdmin>>("formEntries", {
  images: { maxAmount: 1 },
  schema: catalogAdminSchema,
  lang,
  emptyItem: {
    images: [],
    link: "",
  ...
  handleSubmit: async (state: Ref<CatalogAdmin>): Promise<{ error?: string | undefined }> => {
    ...
    formData.set("catalogAdminItemJson", JSON.stringify(state.value))
...
provide<StoreType>("storeType", "catalogAdminsStore")
</script>
