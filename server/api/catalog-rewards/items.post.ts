...
export default defineEventHandler(async (event): Promise<{ error?: string }> => {
  ...
  const [db_item] = await queries().catalogRewards.create(itemJson)

  handleImages({ imagesBefore: [], imagesAfter: itemJson.images, refer_id: db_item.id, refer_type: "catalog-reward" })
